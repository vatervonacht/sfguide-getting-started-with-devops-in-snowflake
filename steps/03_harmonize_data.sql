...
The SnowflakeFile class provides dynamic file access, to stream files
of any size.
 */
...
-- To obtain these, we need to divide the emission data by the number
-- of seats in the airplane.
create or replace view flight_emissions as
select
    departure_airport,
    arrival_airport,
    avg(estimated_co2_total_tonnes / seats)
    * 1000 as co2_emissions_kg_per_person
from
...
select
    ...
    count(
        case
            when arrival_actual_ingate_timeliness in ('OnTime', 'Early') then 1
        end
    )
    / count(*)
    * 100 as punctual_pct
...
inner join
    flight_punctuality
    on
        departure_airport = departure_iata_airport_code
        and arrival_airport = arrival_iata_airport_code
where
    departure_airport = (select $1:airport from @quickstart_common.public.quickstart_repo/branches/main/data/home.json (file_format => bronze.json_format));
...
-- US cities with atleast 100k residents to enjoy all the benefits
-- a big city provides during our vacation.
create or replace view major_us_cities as
select
    geo.geo_id,
    geo.geo_name,
    max(ts.value) as total_population
from government_essentials.cybersyn.datacommons_timeseries as ts
inner join
    government_essentials.cybersyn.geography_index as geo
    on ts.geo_id = geo.geo_id
inner join
    government_essentials.cybersyn.geography_relationships as geo_rel
...
from
    us_points_of_interest__addresses.cybersyn.geography_relationships as country
inner join
    us_points_of_interest__addresses.cybersyn.geography_relationships as city
...
select
    ...
    avg(avg_temperature_air_f) as avg_temperature_air_f,
    avg(avg_relative_humidity_pct) as avg_relative_humidity_pct,
    avg(avg_cloud_cover_pct) as avg_cloud_cover_pct,
    avg(precipitation_probability_pct) as precipitation_probability_pct
from major_us_cities as city
inner join zip_codes_in_city as zip on city.geo_id = zip.city_geo_id
inner join weather_forecast as weather on zip.zip_geo_name = weather.postal_code
